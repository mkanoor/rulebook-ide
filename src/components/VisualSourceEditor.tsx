...
export const VisualSourceEditor: React.FC<VisualSourceEditorProps> = ({
  ...
  const schemaLoader = useSchemaLoader({
    onSchemaLoaded: (result) => {
      console.log('VisualSourceEditor: onSchemaLoaded called with schema:', result.schema.title);
      setCurrentSchema(result.schema);
      console.log('VisualSourceEditor: currentSchema state updated');
      ...
      console.log('VisualSourceEditor: sourceArgs updated:', mergedArgs);
      ...
      onChange(newSource);
      console.log('VisualSourceEditor: source updated');
    }
  });

  // Load all available source schemas on mount
  useEffect(() => {
    console.log('VisualSourceEditor: Loading schemas...');
    getAllEventSourceSchemas().then(schemas => {
      console.log('VisualSourceEditor: Loaded schemas:', Object.keys(schemas));
  ...
  useEffect(() => {
    console.log('VisualSourceEditor: useEffect triggered - showSelector:', showSourceTypeSelector, 'isCustom:', isCustomSource, 'customType:', customSourceType);

    // Don't auto-load if we're showing the source type selector
    if (showSourceTypeSelector) {
      console.log('VisualSourceEditor: Showing selector, skipping auto-load');
      return;
    }

    const { name, filters, ...rest } = source;
    const keys = Object.keys(rest);
    console.log('VisualSourceEditor: Source changed, keys:', keys);

    if (keys.length > 0) {
      const type = keys[0];
      const args = rest[type];
      console.log('VisualSourceEditor: Found type:', type, 'args:', args);
      ...
      if (isCustomSource && currentSchema && customSourceType === type) {
        console.log('VisualSourceEditor: Skipping auto-load, using existing custom schema');
        return;
      }
      ...
      loadEventSourceSchema(type).then(schema => {
        if (schema) {
          console.log('VisualSourceEditor: Loaded schema for', type);
          ...
          if (!type.startsWith('ansible.eda.')) {
            console.warn('VisualSourceEditor: No schema found for', type, 'using custom mode');
          }
      ...
      console.log('VisualSourceEditor: No source type found - isCustomSource:', isCustomSource);
      ...
      if (!isCustomSource) {
        console.log('VisualSourceEditor: Not in custom mode, showing selector');
        setShowSourceTypeSelector(true);
      } else {
        console.log('VisualSourceEditor: In custom mode, keeping current state');
      }
    }
  ...
  const handleCustomSourceSelect = () => {
    console.log('VisualSourceEditor: handleCustomSourceSelect called');
    setIsCustomSource(true);
    setShowSourceTypeSelector(false);
    setCurrentSchema(null);
    setCustomSourceType('');
    ...
    console.log('VisualSourceEditor: Custom source state set - isCustomSource=true, showSelector=false, source cleared');
  };
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px', marginTop: '8px' }}>
            {Object.entries(availableSchemas).map(([sourceType, schema]) => (
              ...
              >
                ...
                <small style={{ fontSize: '10px', opacity: 0.8 }}>{sourceType}</small>
              </button>
            ))}
            <button
              type="button"
            ...
            >
              ...
              <small style={{ fontSize: '10px', opacity: 0.8 }}>Manual configuration</small>
            </button>
          </div>
        ...
        <div>
          <div style={{
            ...
            <div>
              <strong>Custom Source</strong>
              <br />
              <small style={{ color: '#718096', fontSize: '12px' }}>Manual configuration</small>
            </div>
            <button
              type="button"
              className="btn btn-secondary btn-small"
          ...
          <div style={{ marginBottom: '20px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '12px' }}>Configuration</h4>
            ...
            {!currentSchema ? (
              <>
                ...
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    rows={10}
                    style={customSourceArgsError ? { borderColor: '#fc8181', borderWidth: '2px' } : {}}
                    placeholder='{\n  "port": 5000,\n  "host": "0.0.0.0"\n}'
                  />
              ...
              <>
                <div style={{ padding: '8px 12px', backgroundColor: '#e6ffed', borderRadius: '4px', marginBottom: '12px', fontSize: '13px', color: '#22543d' }}>
        ...
        <div>
          <div style={{
            ...
            <div>
              ...
              <br />
              <small style={{ color: '#718096', fontSize: '12px' }}>{selectedSourceType}</small>
            </div>
            <button
              type="button"
              className="btn btn-secondary btn-small"
